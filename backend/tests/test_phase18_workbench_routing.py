...
# ---------------------------------------------------------------------------
# 7. General API — unavailable model returns honest UNAVAILABLE response
# ---------------------------------------------------------------------------
def test_general_run_unavailable_returns_honest_response():
    resp = client.post("/api/general/run", json={"task": "hey", "asset_tag": "", "use_rag": False})
    assert resp.status_code == 200
    data = resp.json()
    assert data["status"] == "UNAVAILABLE"
    assert data["actual_model_execution"] == []
    assert data["rag_used"] is False
    assert data["answer"] is None
    assert "General model is not currently available" in (data.get("message") or "")


# ---------------------------------------------------------------------------
# 8. General API — RAG query with absent general still retrieves evidence
# ---------------------------------------------------------------------------
def test_general_run_rag_query_allows_retrieval_without_fake_synthesis():
    ...
    assert data["actual_model_execution"] == []
    assert data["answer"] is None
    assert "controlled reactor shutdown" not in (data.get("message") or "").lower()
